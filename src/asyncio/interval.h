...
template <typename Func> struct Interval {
    ...
    Interval(Func &&callback, Context &context, long long ms, long long start_time)
...
// func should have the signature bool() or void(), return true to continue intervals, false to stop, or void for always
template <typename Func> void make_interval(Context &context, long long ms, Func &&func) {
    make_interval_at(context, ms, current_time_ms() + ms, std::forward<Func>(func));
}

// func should have the signature bool() or void(), return true to continue intervals, false to stop, or void for always
// first call will be at start_time
template <typename Func> void make_interval_at(Context &context, long long ms, long long start_time, Func &&callback) {
